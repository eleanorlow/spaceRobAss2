...
class GraphSearch:
    ...
    def naive_path_planner(self, start_idx, goal_idx):
        ...
        node_goal = self.graph_.nodes_[goal_idx]
        ...
        while current.idx != goal_idx:
            ...
            ####################
            best_neighbour = None
            best_neighbour_distance = 99999

            for neighbour in current.neighbours:
                count_visits = path.count(neighbour)

                if count_visits < 3:
                    distance_to_goal = math.sqrt((node_goal.x - neighbour.x)**2 + (node_goal.y - neighbour.y)**2) + 100 * count_visits
        ...
        while len(unvisited_set) > 0:
            ...
            for neighbour_idx in range(len(self.graph_.nodes_[node_idx].neighbours)):
                ...
                if neighbour.idx in visited_set:
                    ...
                    tentative_cost_to_node = current_node.cost_to_node + neighbour_edge_cost
                    heuristic_score = neighbour.distance_to(self.graph_.nodes_[goal_idx])
                    tentative_cost_to_node_to_goal_heuristic = tentative_cost_to_node + self.heuristic_weight_ * heuristic_score
